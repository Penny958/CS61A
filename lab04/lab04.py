...
def distance(city1, city2):
    ...
    5.0
    """
    differ_x = get_lat(city1) - get_lat(city2)
    differ_y = get_lon(city1) - get_lon(city2)
    return sqrt(differ_x ** 2 + differ_y ** 2)
...
def closer_city(lat, lon, city1, city2):
    ...
    dis_c1 = distance(base,city1)
    dis_c2 = distance(base,city2)
    if dis_c1 <= dis_c2:
...
def replace_elem(lst, index, elem):
    ...
    new = [lst[i] if i != index else elem for i in range(0,len(lst))]
    return new
